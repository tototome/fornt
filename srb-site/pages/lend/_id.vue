...
export default {
  ...
  //this.$route.params.id 所以必须是params
  //异步函数 不影响后面代码执行
  async asyncData({ $axios, params }) {
    let response = await $axios.$get(
      "/api/core/lend/getLendInfoDetail/" + params.id
    );
    return {
  ...
  methods: {
    fetchData() {
      ...
      this.userAccountById();
      this.getLendItemList();
    },
    userAccountById() {
      this.$axios.$get("/api/core/lend/userAccountById").then((response) => {
        this.account = response.data.amount;
      });
    },
    getInterestCount() {
      // 四个参数 还款方式 期数 年利率  借款金额
      this.$axios
        .$get(
          `/api/core/lendItem/getInterestCount/${this.invest.investAmount}/${this.lend.lendYearRate}/${this.lend.period}/${this.lend.returnMethod}`
        )
        .then((response) => {
    ...
    getLendItemList(){
      this.$axios.$get("/api/core/lendItem/getLendItemList")
      .then((reponse)=>{
        this.lendItemList=reponse.data.lendItemList
      })
    },
    commitInvest() {
      //数据提交
